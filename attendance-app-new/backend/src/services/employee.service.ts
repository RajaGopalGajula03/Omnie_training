...
export const getEmployeesService = async () => {
    const [rows] = await db.execute<RowDataPacket[]>(
        `SELECT id,name,email,role FROM employees WHERE deleted_at IS NULL ORDER BY created_at DESC`
    );
...
export const updateEmployeeService = async (employeeId: number, data: UpdateEmployeeData, userId: number) => {
    ...
    const [result] = await db.execute<ResultSetHeader>(
        ...
        updated_at = NOW() WHERE id = ? `, [name, email, role, department_id, userId, employeeId]
    )
