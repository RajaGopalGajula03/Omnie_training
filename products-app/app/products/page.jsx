...
export default function Products() {
    ...
    useEffect(() => {
        const getProducts = async () => {
            ...
            const url = `${baseUrl}${separator}limit=${limit}&skip=${skip}`
                    ...
                    <Grid item xs={12} sm={6} md={3} key={product.id} display="flex">
                        <Card sx={{ width: '100%', display: "flex", flexDirection: 'column' }}>
                            ...
                            <Box sx={{ display: 'flex', justifyContent:'center', mt:'auto' }} mb={1} gap={2}>
                                ...
                                <Button variant="contained" color="warning" onClick={() => { router.push(`/products/${product.id}`) }}>Edit</Button>
                                <Button variant="contained" color="error" onClick={() => { router.push(`/products/${product.id}`) }}>Delete</Button>
                            </Box>
                        </Card>
                    </Grid>
                ))}
